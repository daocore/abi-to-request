...
export var ContractRequestContextProvider = function (_a) {
    ...
    var abisData = useMemo(function () { return isEmpty(abis) || !abis ? require("./abis.js") : abis; }, [abis]);
    ...
    useEffect(function () {
        if (!library || isEmpty(abisData) || !networkId)
            return;
        var abisFilter = filter(abisData, function (item) { return Number(item.chainId || item.networkId) === Number(networkId); });
