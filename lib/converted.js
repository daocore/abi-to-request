...
const { entry, output, isJs, chainId, contractName, npmjs, isDelete, isBigNumber, exportApis } = config;
	...
	async.forever(
		function (callback) {
			...
			if (convertedAmount >= fileAmount) {
				if (exportApis) {
					writeFile(`abis.${isJs ? "js" : "ts"}`, `${isJs ? "" : `import { TAbiItem } from "abi-to-request";`}

export const abis${isJs ? "" : ": TAbiItem[]"} = ${JSON.stringify(abisFile)}`);
				}

				fs.writeFileSync(path.join(__dirname, `/contract/abis.js`), `module.exports = ${JSON.stringify(abisFile)}`);
	...
	if (content instanceof Array) {
		...
		for (let i in content?.networks) {
			abis.push({
				...
				abi: content?.abi,
				networkId: i,
				address: content?.networks[i]?.address,
				frame: "truffle"
			})
		}
	} else {
...
const toRequestFile = (abis) => {
	for (let i = 0; i < abis?.length; i++) {
		...
		let fileStr = isJs ? "" : `${npmjs != "etherjs" && npmjs != "web3js"
			? `import { TContract${!isBigNumber ? ", convertedBigNumber" : ""} } from 'abi-to-request';
...
`
		...
		const contractFileName = chainId ? `${abis[i].contractName}` : `${abis[i].contractName}_${lodash.upperFirst(lodash.camelCase(name))}`
...
`
	...
	if (${contractTypeAny}?.address && !${contractTypeAny}?.methods) {
		...
		return ${!isBigNumber && etherjsReturns ? "convertedBigNumber(res)" : "res"}${etherReturn}
	}
}
`
			} else {
